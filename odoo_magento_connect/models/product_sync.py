...
class ConnectorSnippet(models.TransientModel):
    ...
    def _sync_magento2_template_variants(self, template_obj, template_sku, instance_id, channel, url, token, connection):
        mage_variant_ids = []
        ctx = dict(self._context or {})
        for vrnt_obj in template_obj.product_variant_ids:
            ...
            if exist_map_obj:
                ...
                mage_vrnt_id = self._export_magento2_specific_product(
    ...
    def _get_magento2_product_media(self, prod_obj):
    ...
    def _get_magento2_product_array(self, instance_id, channel, prod_obj, get_product_data, connection):
        prod_categs = []
        ...
        if 'custom_attributes' not in get_product_data :
            ...
            get_product_data['custom_attributes'] += custom_attributes
        image_data = self._get_magento2_product_media(prod_obj)
        if image_data:
            get_product_data.update(media_gallery_entries=[image_data])
        return get_product_data
    
    def _get_magento2_product_qty(self, prod_obj, instance_id, stock_item_id=False, stock_id=1):
    ...
    #############################################
    def _export_magento2_specific_product(self, vrnt_obj, template_sku, instance_id, channel, url, token, connection):
        """
        ...
        if vrnt_obj:
            ...
            stock_data = self._get_magento2_product_qty(vrnt_obj, instance_id)
            ...
            get_product_data = self._get_magento2_product_array(
            ...
            if prod_response and prod_response.get('id'):
                ...
                self.create_odoo_connector_mapping(
                    'connector.product.mapping', ecomm_id, odoo_id, instance_id, 
                    magento_stock_id=prod_response['extension_attributes']['stock_item']['item_id']
                )
    ...
    def _export_magento2_specific_template(self, exup_product_obj, instance_id, channel, connection):
        ...
        if status and exup_product_obj:
            ...
            if not exup_product_obj.product_variant_ids:
                ...
                if not wk_attr_line_objs:
                    ...
                    mage_prod_ids = self.with_context(ctx)._sync_magento2_template_variants(
                    ...
                    if not mage_set_id:
                        ...
                        get_product_data = self.with_context(ctx)._get_magento2_product_array(
    ...
    #############################################
    def _update_magento2_specific_product(self, prod_map_obj, url, token, channel, connection):
        get_product_data = {}
        ...
        if prod_obj and mage_prod_id:
            ...
            if instance_obj.inventory_sync == 'enable':
                stock_data = self._get_magento2_product_qty(prod_obj, instance_obj.id)
    ...
    def _update_magento2_specific_template(self, exup_product_obj, instance_id, channel, connection):
        status, ecomm_id, error = connection.get('status', False), 0, ''
        if status and exup_product_obj:
            url = connection.get('url', '')
            token = connection.get('token', '')
            ...
            if temp_obj and mage_prod_id:
                if temp_obj.product_variant_ids:
                    ...
                    mage_prod_ids = self._sync_magento2_template_variants(temp_obj, template_sku, instance_id, channel, url, token, connection)
                    for vrnt_obj in temp_obj.product_variant_ids:
                        ...
                        if prod_map_obj:
                            self._update_magento2_specific_product(prod_map_obj, url, token, channel, connection)
                else:
                    return {'status': 0, 'error': str(temp_obj.id) + ' No Variant Ids Found!!!'}
                if exup_product_obj.is_variants and mage_prod_ids:
                    get_product_data = self._get_magento2_product_array(instance_id, channel, temp_obj, get_product_data, connection)
